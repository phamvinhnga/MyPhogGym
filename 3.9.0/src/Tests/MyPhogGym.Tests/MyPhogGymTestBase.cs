...
using MyPhogGym.Migrations.SeedData;
...
namespace MyPhogGym.Tests
{
    public abstract class MyPhogGymTestBase : AbpIntegratedTestBase<MyPhogGymTestModule>
    {
        ...
        protected void UsingDbContext(Action<MyPhogGymDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MyPhogGymDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MyPhogGymDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MyPhogGymDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MyPhogGymDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyPhogGymDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MyPhogGymDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyPhogGymDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MyPhogGymDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MyPhogGymDbContext, Task<T>> func)
        {
            T result;
